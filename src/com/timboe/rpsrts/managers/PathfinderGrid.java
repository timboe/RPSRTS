	...
	public void Init() {
		...
		for (int x = -utility.world_size2; x < utility.world_size2; x = x + utility.tiles_size) {
			for (int y = -utility.world_size2; y < utility.world_size2; y = y + utility.tiles_size) {
				...
				final WeightedPoint WP = new WeightedPoint(x+(utility.tiles_size/2), y+(utility.tiles_size/2));
		...
		for (final WeightedPoint _P : point_collection) {
			for (int neighbour = 0; neighbour < 8; ++neighbour) { //TODO check nn vs nnn
				...
				final WeightedPoint _n = point_collection_map.get(new WorldPoint(neighbour_x, neighbour_y));
