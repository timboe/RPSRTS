...
public class GameWorld_App extends GameWorld {
	...
	public void DrawChunks(final Canvas canvas, final boolean _useEnergy, final boolean _aa) {
	...
	public void DrawIslandEdge(final Canvas canvas) {
		...
		for (final long edge : island_offset) {
			...
			final RectF _rect = new RectF(-new_radius, -new_radius, new_radius, new_radius);
			int toDraw = angle + 30;
	...
	public void DrawTiles(final Canvas canvas, final boolean _renderAll, final boolean _aa) {
		if (_renderAll == true) {
