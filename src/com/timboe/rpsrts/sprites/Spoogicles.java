...
public class Spoogicles extends Sprite {
	...
	protected Spoogicles(final int _ID, final int _x, final int _y, final ObjectOwner _oo, final int _spoogicles, final float _scale) {
		super(_ID, _x, _y, 1);
		...
		for (int s = 0; s < _spoogicles; ++s) {
			...
			spooge_x_vel.add( new AtomicFloat( (utility.rndG(0f, 2f * scale) )) );
			spooge_y_vel.add( new AtomicFloat( (float) (-3.5 - Math.abs(utility.rndG(0f, 2f * scale))) ));
